...
quat::quat() : v(), s(1.0) {}                                            // No rotation (identity) by default
quat::quat(double X, double Y, double Z, double S) : v(X, Y, Z), s(S) {} // Take care of order
quat::quat(const vec3r &V, const double S) : v(V), s(S) {} // Be carreful! This is NOT axis with angle of rotation!!
...
/// Product operation between two quaternion (note that q1*q2 is generally not equal to q2*q1)
/// In term of rotations, the rotation q2 is first applied and then q1
quat operator*(const quat &q1, const quat &q2) // OP: 14*, 10±
{
...
vec3r quat::operator*(const vec3r &V) const {
...
quat quat::dot(const vec3r &omega) {
...
void quat::conjugate() { v = -v; }
quat quat::get_conjugated() const { return quat(-v.x, -v.y, -v.z, s); }
...
void quat::set_axis_angle(const vec3r &V, double angle) {
...
void quat::set(double X, double Y, double Z, double S) {
  v.set(X, Y, Z);
  s = S;
}

/// @brief Return the rotation angle
double quat::get_angle() const { return (2.0 * acos(s)); }

/// @brief Return Pitch angle (along x in DEMbox reference frame)
double quat::get_Pitch() const { return atan2(2.0 * (v.y * v.z + s * v.x), s * s - v.x * v.x - v.y * v.y + v.z * v.z); }

/// @brief Return Yaw angle (along y in DEMbox reference frame)
double quat::get_Yaw() const { return asin(-2.0 * (v.x * v.z - s * v.y)); }

/// @brief Return Roll angle (along z in DEMbox reference frame)
double quat::get_Roll() const { return atan2(2.0 * (v.x * v.y + s * v.z), s * s + v.x * v.x - v.y * v.y - v.z * v.z); }

/// @brief Return the rotation axis
vec3r quat::get_axis() const {
  vec3r Axis;
...
void quat::set_from_to(const vec3r &V1, const vec3r &V2) {
...
void quat::TwistSwingDecomp(const vec3r &V1, quat &twist, quat &swing) {
...
/// @see http://xrual.googlecode.com
void quat::SwingTwistDecomp(const vec3r &V1, quat &swing, quat &twist) {
  vec3r V2 = (*this) * V1;
  swing.set_from_to(V1, V2);
  twist = swing.get_conjugated() * (*this);
}

/// @brief Make the quaternion normalized and return its length (before normalization of course!)
double quat::normalize() {
...
void quat::get_rot_matrix(double M[]) const // OP: 12*, 6+, 6-
{
  double Tx = 2.0 * v.x;
  double Ty = 2.0 * v.y;
  double Tz = 2.0 * v.z;
...
void quat::get_rot_matrix(mat9r &M) const // OP: 12*, 6+, 6-
{
  double Tx = 2.0 * v.x;
  double Ty = 2.0 * v.y;
  double Tz = 2.0 * v.z;
...
int quat::set_rot_matrix(double m[]) {
...
mat9<double> quat::rotate_diag_mat(const vec3r &u) const {
  double P[9];
  get_rot_matrix(P);
  mat9<double> Res;
...
vec3r quat::rotate(const vec3r &u) const // OP: 21*, 18±
{
  double M[9];
...
vec3r quat::unrotate(const vec3r &u) const {
  double M[9];
  get_rot_matrix(M);
  vec3r v;
...
// --- input/output ---
std::ostream &operator<<(std::ostream &pStr, const quat &Q) {
  pStr << Q.s << ' ' << Q.v;
  return pStr;
}

std::istream &operator>>(std::istream &pStr, quat &Q) {
  pStr >> Q.s >> Q.v;
  return pStr;
}
