...
void MPMbox::updateVelocityGradient() {
  ...
  for (size_t p = 0; p < MP.size(); p++) {
    ...
    for (size_t r = 0; r < element::nbNodes; r++) {
      MP[p].velGrad.xx += (MP[p].gradN[r][0] * nodes[I[r]].vel.x);
      MP[p].velGrad.yy += (MP[p].gradN[r][1] * nodes[I[r]].vel.y);
      MP[p].velGrad.xy += (MP[p].gradN[r][1] * nodes[I[r]].vel.x);
      MP[p].velGrad.yx += (MP[p].gradN[r][0] * nodes[I[r]].vel.y);
    }
  }
}
...
void MPMbox::postProcess(std::vector<ProcessedDataMP> &Data) {
  ...
  for (size_t p = 0; p < MP.size(); p++) {
    ...
    for (size_t r = 0; r < element::nbNodes; r++) {
      ...
      Data[p].velGrad.xx += (MP[p].gradN[r][0] * nodes[I[r]].vel.x);
      Data[p].velGrad.yy += (MP[p].gradN[r][1] * nodes[I[r]].vel.y);
      Data[p].velGrad.xy += (MP[p].gradN[r][1] * nodes[I[r]].vel.x);
      Data[p].velGrad.yx += (MP[p].gradN[r][0] * nodes[I[r]].vel.y);
