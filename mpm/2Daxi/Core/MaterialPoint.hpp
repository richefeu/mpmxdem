...
struct MaterialPoint {
  ...
  double outOfPlaneEp{0.0}; // Out-of-plane plastic strain component
  mat4r deltaStrain;        // Increment of strain (it is computed in ConstitutiveModel for processing purpose)
                            // (FIXME: remove?)
  double outOfPlaneStrain{0.0}; // Out-of-plane total strain component (=epsθθ)
  double outOfPlaneDeltaStrain{0.0};// Out-of-plane total strain increment (=depsθθ)
  ...
  double outOfPlaneStress{0.0}; // Out-of-plane total stress component (= sigmaθθ)
  ...
  double gradN[16][3]; // Gradient of shape function according to the position of the Material Point in axisymmetric geometry (dN/dr,dN/dy,N/r)
