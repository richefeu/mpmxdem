...
int ModifiedLagrangian::advanceOneStep(MPMbox& MPM) {
  ...
  for (size_t p = 0; p < MP.size(); p++) {
    ...
    for (size_t r = 0; r < element::nbNodes; r++) {
      // Internal forces (formula from Sulsky & Schreyer 1996 https://doi.org/10.1016/S0045-7825(96)01091-2 )
      nodes[I[r]].f.x += - MP[p].vol * (MP[p].stress.xx * MP[p].gradN[r][0] + MP[p].stress.xy * MP[p].gradN[r][1] 
                         + MP[p].outOfPlaneStress * MP[p].gradN[r][2]);
      nodes[I[r]].f.y += - MP[p].vol * (MP[p].stress.xy * MP[p].gradN[r][0] + MP[p].stress.yy * MP[p].gradN[r][1]); 
      // if(p == 10){std::cout<<r<< " " << MP[p].gradN[r][0]<<std::endl;}

      // External forces (gravity)
      nodes[I[r]].f += MP[p].mass * MPM.gravity * MP[p].N[r];
    }
    // if(p == 10){std::cout<< "outOfPlaneStress : " << MP[p].outOfPlaneStress<<std::endl;}
  }
