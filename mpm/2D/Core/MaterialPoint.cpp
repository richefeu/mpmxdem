...
MaterialPoint::MaterialPoint(int Group, double Size, double Rho, ConstitutiveModel *CM)
    : groupNb(Group), mass(0.0), size(Size), vol0(0.0), vol(0.0), density(Rho), pos(), vel(), securDist(0.0), f(), q(),
      strain(), plasticStrain(), outOfPlaneEp(0.0), deltaStrain(), stress(), outOfPlaneStress(0.0), hardeningForce(0.0),
      e(0), F(), velGrad(), splitCount(1), prev_pos(), plastic(false), contactf(), constitutiveModel(CM),
      isDoubleScale(false), PBC(nullptr), isTracked(false) {
  vol0 = vol = size * size;
  mass       = vol0 * density;
  F          = mat4r::unit();
  prev_F     = F;
