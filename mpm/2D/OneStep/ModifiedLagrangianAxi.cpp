...
#include "ModifiedLagrangianAxi.hpp"
...
std::string ModifiedLagrangianAxi::getRegistrationName() { return std::string("ModifiedLagrangianAxi"); }
...
// 
// We consider (x,y) as (r,y) where r = x-x_ax in an axisymmetric configuration with axis position x_ax

int ModifiedLagrangianAxi::advanceOneStep(MPMbox& MPM) {
  ...
  for (size_t n = 0; n < liveNodeNum.size(); n++) {
    ...
    nodes[liveNodeNum[n]].outOfPlaneStress = 0.0; // in fact orthoradial stress
    nodes[liveNodeNum[n]].q.reset();              // mass*(dr/dt,dy/dt)
    nodes[liveNodeNum[n]].qdot.reset();           // mass*(d2r/dt2,d2y/dt2)
    nodes[liveNodeNum[n]].f.reset();              // (fr,fy)
    nodes[liveNodeNum[n]].fb.reset();             // Boundary forces (fbr,fby)
    nodes[liveNodeNum[n]].vel.reset();            // Velocity (dr/dt,dy/dt)
  }
