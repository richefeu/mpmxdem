#include "SinfoniettaCrush.hpp"

#include "Core/MPMbox.hpp"
#include "Core/MaterialPoint.hpp"

std::string SinfoniettaCrush::getRegistrationName() { return std::string("SinfoniettaCrush"); }

SinfoniettaCrush::SinfoniettaCrush(double young, double poisson)
    : Young(young), Poisson(poisson), C(young, poisson), kappa(0.0), varphi(0.0), pc0(0.0) {}

void SinfoniettaCrush::read(std::istream& is) {
  double varphiDeg;
  is >> Young >> Poisson >> beta >> beta_p >> kappa >> varphiDeg >> pc0;
  is >> phiStar0 >> epv0 >> l0 >> ginf;
  bfunc.set(ginf, epv0, l0);
...
void SinfoniettaCrush::write(std::ostream& os) {
...
mat9r SinfoniettaCrush::func_df_ds(mat9r Sigma, double /*q*/) {
...
mat9r SinfoniettaCrush::func_dg_ds(mat9r Sigma, double /*q*/) {
  double p = -Sigma.trace() / 3.0 + epsilon_pressure;
  mat9r s = Sigma + p * mat9r::unit();
  mat9r xi = s * (1.0 / p);
  double J2xi = inner_product(xi, xi);
...
double SinfoniettaCrush::func_df_dq(mat9r /*Sigma*/, double /*q*/) { return 3.0 * beta * (z - 3.0); }

double SinfoniettaCrush::func_h(mat9r Sigma, double q, mat9r Ep) {
  mat9r dg_ds = func_dg_ds(Sigma, q);
  mat9r dev_dg_ds = deviatoric(dg_ds);
  return -dg_ds.trace() * (1.0 - phiStar0 * bfunc.deriv(-Ep.trace())) + kappa * sqrt(inner_product(dev_dg_ds, dev_dg_ds));
}

void SinfoniettaCrush::updateStrainAndStress(MPMbox& MPM, size_t p) {
...
double SinfoniettaCrush::getYoung() { return Young; }

double SinfoniettaCrush::getPoisson() { return Poisson; }
