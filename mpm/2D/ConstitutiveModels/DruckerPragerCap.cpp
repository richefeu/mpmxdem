...
std::string DruckerPragerCap::getRegistrationName() { return std::string("DruckerPragerCap"); }
...
DruckerPragerCap::DruckerPragerCap(double young, double poisson, double frictionAngle, double cohesion, double hydroConsolStress,
 double capEccentricity) : E0(young), Nu0(poisson), Beta(frictionAngle), d0(cohesion), Pb0(hydroConsolStress), R0(capEccentricity) {
  tanBeta = tan(Beta); 
...
double DruckerPragerCap::getYoung() { return E0; }

double DruckerPragerCap::getPoisson() { return Nu0; }
...
void DruckerPragerCap::updateStrainAndStress(MPMbox& MPM, size_t p) {
  ...
  else{ // or update DPC material properties of the MP (expressions are derived from experimental regressions from B. Spanu et al.)
  ...
  // std::cout<<"DruckerPragerCap : MP n°"<<p<<", P : " << P << ", Q : " << Q << ", yieldF : "<<yieldF<< std::endl;
}
