...
#include "frictionalViscoElastofragile.hpp"
...
#include <algorithm>
static Registrar<BoundaryForceLaw, frictionalViscoElastofragile> registrar("frictionalViscoElastofragile");

void frictionalViscoElastofragile::computeForces(MPMbox &MPM, size_t o) {
  ...
  for (size_t nn = 0; nn < MPM.Obstacles[o]->Neighbors.size(); ++nn) {
    ...
    if (dn < gap) {
      ...
      if (dn < 0) { // Check if there is contact
        ...
        MPM.Obstacles[o]->Neighbors[nn].fn = std::max(0.0, -visc * normalVel);
