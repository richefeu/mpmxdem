#include "CircleInside.hpp"
#include <Core/MPMbox.hpp>
#include <Core/MaterialPoint.hpp>

#include <factory.hpp>
static Registrar<Obstacle, CircleInside> registrar("CircleInside");

// Ctor
CircleInside::CircleInside() {}

CircleInside::CircleInside(vec2r Pos, double Radius) {  // used when creating objects within the code
  group = 1;
  pos.x = Pos.x;
  pos.y = Pos.y;
  R = Radius;
...
void CircleInside::read(std::istream& is) {
  is >> group >> pos >> R;
  ...
  if (driveMode == "freeze") {
    ...
    std::cerr << "@CircleInside::read, driveMode " << driveMode << " is unknown!" << std::endl;
  }
}

int CircleInside::touch(MaterialPoint& MP, double& dn) {
...
void CircleInside::getContactFrame(MaterialPoint& MP, vec2r& N, vec2r& T) {
  vec2r N1 = pos - MP.pos;
...
void CircleInside::checkProximity(MPMbox& MPM) {
...
int CircleInside::addVtkPoints(std::vector<vec2r>& coords) {
...
bool CircleInside::MPisInside(MaterialPoint& MP) {
  vec2r l = MP.pos - pos;
